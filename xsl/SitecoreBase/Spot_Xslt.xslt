...
<xsl:template match="*" mode="main">
  ...
  <div class="leftspot">
    <div class="spotheading">
      <sc:text field="spot heading" />
    </div>
    <xsl:if test="sc:fld('spot text',.) != ''">
      <div class="spottext">
        <sc:text field="spot text" />
      </div>
    </xsl:if>
    <xsl:if test="sc:fld('spot image',.) != ''">
      <div class="spotimage">
        <xsl:choose>
          <xsl:when test="sc:fld('spot link',.) = ''">
            <sc:image field="spot image" w="170"/>
          </xsl:when>
          <xsl:otherwise>
            <sc:link field="spot link">
              <sc:image field="spot image" w="170"/>
            </sc:link>
          </xsl:otherwise>
        </xsl:choose>        
    ...
    <xsl:if test="sc:fld('spot read more text',.) != ''">
      <div class="readmore">
        <sc:link field="spot link">
